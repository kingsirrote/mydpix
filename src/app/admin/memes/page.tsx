...
export default async function AdminMemesPage() {
  ...
  const { data: memes } = await service
    .from("memes")
    .select("id, title, thumbnail_url, image_url, moderation_status, is_featured, owner_id, created_at")
        ...
        {(memes ?? []).map((meme) => (
          <div key={meme.id} className="overflow-hidden rounded-2xl border border-base-700 bg-base-900">
            ...
            <div className="p-3">
              ...
              <p className="mt-1 text-[10px] uppercase tracking-wide text-ink-500">{meme.moderation_status}</p>
